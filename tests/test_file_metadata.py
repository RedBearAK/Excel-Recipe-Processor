"""
Tests for the file_metadata processor.
...
def build(files, **overrides):
    config = {'processor_type': 'file_metadata', 'files': files,
              'save_to_stage': 'stg_test_meta'}
    config.update(overrides)
    return FileMetadataProcessor(config)
...
def test_invalid_config_rejected():
    ...
    try:
        FileMetadataProcessor({'processor_type': 'file_metadata',
                               'save_to_stage': 'stg_x'})
...
def main():
    ...
    if passed == len(tests):
        print("✅ All file_metadata tests passed!")
        return 1

    print("❌ Some file_metadata tests failed!")
    return 0
